#!/usr/bin/env python3
"""
RDODI Pipeline Validator v1.3.0
================================
...
def gate_shacl(gate_id, data_paths, shacl_path):
    # v1.3.0 (L-66 fix): a SHACL Warning is advisory, not a failure. Verdict is driven by
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="RDODI Pipeline Validator v1.3.0")
    ap.add_argument("--config", required=True)
    args = ap.parse_args()
    cfg = json.load(open(args.config))
    print("="*74); print("RDODI PIPELINE VALIDATOR v1.3.0  (L-66 severity-honest SHACL; real gate IDs; rule-based coverage+substance)")
    print("="*74)
