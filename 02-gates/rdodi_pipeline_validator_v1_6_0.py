#!/usr/bin/env python3
"""
RDODI Pipeline Validator v1.6.0
================================
Mechanical gate runner for the RDODI four-stage pipeline (Research -> Domain ->
Document -> Interactive Page).

CHANGES FROM v1.5.0 (MINOR bump per BP-D7; Stage3.F wired in for the first time):
  - Stage3.F (the reference-completeness rule) was named in the procedure's own gate
    table since reference_rule_gate_v1_0_0.py was authored, but never called anywhere
    in validate() -- unlike Stage1.B, the gate function itself was already correct and
    already had a passing adversarial test (6/6, re-verified this session unchanged);
    only the TTL-to-input bridge was missing. Found and proposed by the
    code-abundance-rdodi session (Proposal_RDODI_Stage3F_Wiring_v1_0_0.md,
    github.com/altunelyusuf/RDODI-Research), independently re-verified this session
    (both the PASS and FAIL claims reproduced from scratch, not trusted from the
    proposal's own text) before wiring stage3f_ttl_bridge_v1_0_0.py in via the
    existing reference_rule() function -- no new gate mechanism, an existing one made
    reachable (L-105).
  - HONEST, DISCLOSED LIMIT: this package's own canonical worked example
    (the excluded worked-example material) carries no
    skos:definition anywhere -- Stage3.F wired against it finds 0 claims and passes
    vacuously, the same silent-pass mechanism already found twice this session for
    other reasons. NOT fixed in this pass (populating real cited content into that material's
    25 sections is separate, larger work); a minimal, purpose-built fixture pair
    (fixtures/stage3f_document_positive/negative_v1_0_0.ttl) proves the WIRING itself
    is real, independent of that material's own content gap.
...
def gate_reference_rule(gate_id, document_ttl_path, research_ttl_path):
    """Stage3.F: wires stage3f_ttl_bridge_v1_0_0.extract_ttl_to_reference_rule_input() into
    reference_rule_gate_v1_0_0.reference_rule(). Both pre-existing; this function is the
    missing connection between them (see CHANGES above)."""
    from stage3f_ttl_bridge_v1_0_0 import extract_ttl_to_reference_rule_input
    from reference_rule_gate_v1_0_0 import reference_rule
    refs, claims = extract_ttl_to_reference_rule_input(document_ttl_path, research_ttl_path)
    result = reference_rule(refs, claims)
    return GateResult(gate_id, "STANDARD", result.verdict, result.detail,
                      {"uncited": len(result.uncited), "unverified": len(result.unverified),
...
def validate(cfg):
    ...
    R.append(gate_reference_rule("Stage3.F", cfg["document_ttl"], cfg["research"]))
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="RDODI Pipeline Validator v1.6.0")
