...
A domain ontology — a TBox + ABox + SHACL triple — covering the subject's concepts as discovered in the research artefact's concept inventory. Conforms to the methodology framework at `http://example.org/rdodi/domain-ontology` [source: 01-stage-vocabularies/02-domain/domain_ontology_tbox_v1_2_0.ttl (corrected 2026-09-10, PATCH-equivalent citation fix: the prior citation named `rdodi-v1_0_1/...v1_0_1.ttl`, a path and version that no longer match the shipped file at all)]. **v1.4.0, per HANDOVER_pib-vaf-rdodi_to_RDODI_domain-ontology-scope-gap_v1_0_0.md:** the ABox must also instantiate exactly one `rdodi-domain:DomainOntologyArtifact` individual for the whole subject ontology, declaring `hasCompetencyQuestion`, `hasSourceProvenance`, and `hasReusabilityScope` — enforced as real `sh:Violation`-severity requirements as of `domain_ontology_shacl_v1_2_0.ttl` (the two older checks for the first pair existed since v1.0.0/v1.1.0 but were `sh:minCount 0`/`sh:Info` — decorative, could never fire — found by testing against a real, already-shipped domain corpus that had none of the three and nothing caught it).
...
**v1.6.0, syntactic/mechanic layer for non-code corpora:** where Stage 1's input has no upstream parser resolving its structure the way source code has an AST parser (a requirements document, a design document, an ADR), run `02-gates/docling_corpus_bridge_v1_0_0.py` against it before Stage 2 concept extraction. Chosen after a real, sourced comparison (GROBID/MinerU/Unstructured/Marker) and a real test-drive (`02-gates/fixtures/docling-corpus/`, 2026-09-15: 76 headings across all nesting levels and 248 table rows recovered from a real source document, zero manual configuration). The bridge produces a real `rdodi-domain:DomainCorpus` individual with genuine provenance (source SHA-256, Docling version, output SHA-256) -- it structures the corpus; it does not extract concepts itself, which remains a separate `CorpusDrivenMethod` activity. For a code-bearing corpus, Stage 2 already has its own real parser (the language's own AST) -- this bridge is for the artifact types that don't.
...
3a. **(v1.4.0)** In the same ABox, declare one `rdodi-domain:DomainOntologyArtifact` individual for the whole subject ontology, carrying:
    - `hasCompetencyQuestion` → a `CompetencyQuestionSet` with ≥1 real `CompetencyQuestion` the ontology should be able to answer (not filler — a question an assessor could actually put to the finished ontology).
    - `hasSourceProvenance` → at least one `SourceProvenance` individual disclosing whether the concepts are net-new, corpus-derived, or aligned to an existing vocabulary.
    - `hasReusabilityScope` → `rdodi-domain:RS_SubjectSpecific` or `rdodi-domain:RS_GeneralDomain`, stating plainly whether a different subject's Stage 2 run would recognise these concepts.
...
| Stage2.H | **(v1.4.0)** `DomainOntologyArtifact` individual present, `domain_ontology_shacl_v1_2_0.ttl`'s three artifact-level shapes: 0 `sh:Violation` |

**v1.5.0, for a code-bearing corpus specifically** (per `HANDOVER_automate-python-book_to_RDODI_chapter-domain-ontology-gaps_v1_0_0.md`, 2026-09-15, and `domain_ontology_tbox_v1_3_0.ttl`): where a produced domain concept is an operation on a typed thing (a method, a function bound to a type), declare `hasOwningConcept` linking it to its owner -- an authored operation name is not a checkable claim until this exists (measured: ~94% of bare operation names in a real 313-construct corpus did not resolve to a single owner without it). Where a domain concept's runtime behaviour is claimed, use `hasRuntimeEffect`/`hasErrorCondition`/`hasIOExample` as appropriate -- deliberately distinct from `ipo:`'s pedagogical widget-behaviour vocabulary and from a modelled system's own runtime behaviour (neither sense is being widened or merged; see `RDODI_SEMANTIC_BEHAVIORAL_SYNTHESIS_v1_0_0.md` in `altunelyusuf/VAF`). Declare `hasResolutionEnvironment` on the `DomainOntologyArtifact` (what was installed, at which versions) so two runs over the same corpus that disagree can be attributed to a real, recorded difference rather than left unexplained. See `01-stage-vocabularies/02-domain/RULING_execution-oracle-validation-evidence_v1_0_0.md` for whether an execution-oracle comparison is admissible `ValidationActivity` evidence (yes, via the existing PROV-O attachment mechanism -- no new property was needed).
...
| Stage3.G | ≥25% of substantive paragraphs carry in-line `(Surname, Year)` citations **(parenthetical-form only; not a completeness measure — see Stage3.F for whether every claim is cited, in any form)** |
...
**Default deliverable (v1.3.0, owner ruling 2026-08-27):** a rich, single-file interactive
page — hierarchical left sidebar tree, top+sub tabs, and a grounded chat/prompt console over
the artifact's own content — per the pattern demonstrated in this ecosystem's own internal knowledge-base package and
adopted into this package's own `28-interactive-html-surface/08-brsf-corpus-template/`
mechanism (source page + corpus-instantiation script). This supersedes v1.2.2's framing of a
single flowing anchor-navigated page as *the* deliverable; that shape is now a **named
variation**, not the default (see below).

**Honest disclosure, not fixed by this ruling:** the sidebar/tabs pattern is realized as
in-page JS panel-swapping over one HTML file — confirmed (`grep` for routing constructs
returns 0 matches) — not separate routable pages. `ipo:SubPageTabs` and `ipo:PageHierarchy`
explicitly require **routable sub-pages, each with its own location** ("not in-place panel
swaps"), so this default does **not** instantiate those classes correctly, and they remain
genuinely unused vocabulary — a category mismatch, not something to force-fit by mislabeling
JS tabs as `ipo:SubPageTabs`.

1. Render the Stage 3 document's section structure as the page's permanent visible navigation
   (hierarchical sidebar tree by default; BP-D34's inline-pill-bar rule remains the *fallback*
   decision procedure for the Variation below, where its ≤25/26-50/>50 thresholds still apply).
...
**Variation — single flowing anchor-page** (the v1.2.2 default, retained for smaller artifacts
or minimal-dependency contexts where the rich shell's chat console and sidebar tree are not
warranted): BP-D34's inline pill-bar / grouped-sidebar thresholds govern navigation shape;
see `04-worked-example/ch10-current/ch10_quality_companion_interactive_v4_0_0.html` or
`15-presentation/rdodi_presentation_renderer_v1_0_0.py`.

**Variation — native S6 generator** (`06-s6-exemplar/`): a third, RDODI-authored mechanism,
still not reconciled with the corpus-instantiation approach above (open question, unchanged).
