#!/usr/bin/env python3
"""RDODI Artifact Generator v2.3.0 — domain-general + profile-driven.

Supersedes the SCP-hardcoded rdodi_html_generator_v1_0_0 (which fell back to the SCP namespace and
read scp:example/scp:hasVisual). v2.3.0:
            ...
            if area_list:
                ...
                for c in cs[:5]:
                    ...
                    if defn:
                        ...
                        concept_srcs.append(_concept_source(g, c, primary))
                prose = " ".join(parts) if parts else None
                if concept_srcs:
                    sec_source = Literal(concept_srcs[0])   # real per-concept source
...
# ================= v2.3.0 ADDITION: profile-driven PAGE generation (A3-page) =================
...
def generate_page(domain_ttl, profiles_ttl, profile, doc_abox_ttl, out_dir):
    ...
    for sec in gd.subjects(RDF.type, DOC.ReportSection):
        ...
        out.add((r, DCTERMS.source, Literal(docsrc.get(label, primary))))
