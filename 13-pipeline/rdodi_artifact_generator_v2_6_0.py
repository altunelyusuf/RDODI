#!/usr/bin/env python3
"""RDODI Artifact Generator v2.6.0 — domain-general + profile-driven.

Supersedes the SCP-hardcoded rdodi_html_generator_v1_0_0 (which fell back to the SCP namespace and
read scp:example/scp:hasVisual). v2.6.0:
...
EPIST = Namespace("http://example.org/rdodi/epistemic#")
GEN = Namespace("http://example.org/rdodi/gen#")

def _epistemic_type(src):
    """Grounded epistemic typing — never invents precision (L-66)."""
    t = str(src).lower()
    has_v, has_p = "verbatim" in t, "paraphrase" in t
    if has_v and has_p: return EPIST.DeclaredMixed     # domain says verbatim-OR-paraphrase: imprecise
    if has_v: return EPIST.Verbatim
    if has_p: return EPIST.Paraphrase
    return EPIST.Unspecified
...
def generate(domain_ttl, profiles_ttl, profile, out_dir, intent_ttl=None):
    # STEP 0 (B6): consume authorial intent if provided — domain-neutral (concept IRIs).
    INTENT = Namespace("http://example.org/rdodi/intent#")
    must_remaining, excluded_set = [], set()
    if intent_ttl:
        ig = rdflib.Graph(); ig.parse(intent_ttl, format="turtle")
        must_remaining = [c for c in ig.objects(None, INTENT.mustCoverConcept)]
        excluded_set = set(ig.objects(None, INTENT.excludedConcept))
    ...
    # R4 PROV-O deepening: factual generation activity + agent attribution
    import datetime as _dt
    act = GEN["GenerationActivity"]; agent = GEN["RDODIGenerator"]
    out.add((agent, RDF.type, PROV.SoftwareAgent)); out.add((agent, RDFS.label, Literal("RDODI artifact generator v2.6.0")))
    out.add((act, RDF.type, PROV.Activity)); out.add((act, PROV.wasAssociatedWith, agent))
    out.add((act, PROV.used, URIRef(ns.rstrip("#"))))
    out.add((act, PROV.startedAtTime, Literal(_dt.datetime.utcnow().isoformat()+"Z")))
    out.add((report, PROV.wasGeneratedBy, act)); out.add((report, PROV.wasAttributedTo, agent))
    ...
    for name in sec_names:
        ...
        if name.lower() in ("concept sections", "background", "our approach", "results", "the problem"):
            if area_list:
                ...
                # STEP 0: bring uncovered must-cover concepts to the front; drop excluded ones
                cand = [c for c in (list(must_remaining) + list(cs)) if c not in excluded_set]
                ...
                for c in cand[:5]:
                    ...
                    if defn:
                        ...
                        if c in must_remaining: must_remaining = [m for m in must_remaining if m != c]
        ...
        if prose:
            ...
            out.add((sid, EPIST.sourceType, _epistemic_type(sec_source)))   # grounded in the section's OWN source
...
# ================= v2.6.0 ADDITION: profile-driven PAGE generation (A3-page) =================
...
def generate_page(domain_ttl, profiles_ttl, profile, doc_abox_ttl, out_dir):
    ...
    out.add((surface, IP.genre, Literal(genre)))   # v2.6.0: page declares its genre (profile-driven, renderer-readable)
